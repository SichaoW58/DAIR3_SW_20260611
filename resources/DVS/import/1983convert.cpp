//1983convert.cpp by: Derek Onken
//
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {/*9t,*/39,68,98,128,157,187/*t*/,216,246,276,305,335,364/*t*/,393,423,452,482};//jan 1, 1982 = 1, these are the dates of full moons
//jan 9,feb 8,mar 9, apr 8,may 8,jun 6,jul 6,aug 4,sept 3,oct 3, nov 1,dec 1,dec30,jan 28,feb 27,mar 28,apr 27
static int loc=15;

int deltaMoon1983 (int conDate){
	if(conDate < 40){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1983(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1983.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1983.csv",ios::app);
term.open("termPreg1983.csv",ios::app);
gest.open("oldTerm1983.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1982
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1983(conDate);
