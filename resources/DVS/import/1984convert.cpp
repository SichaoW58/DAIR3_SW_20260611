//1984convert.cpp by: Derek Onken
//
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {28,58,87,117,146,176/*p*/,205,235,265,294,324,354/*n*/,383,414,442,471};//jan 1, 1983 = 1, these are the dates of full moons
//jan 28, feb 27, mar 28, apr 27, may 26, jun 25, jul 24, aug 23, sep 22, oct 21, nov 20, dec 20, jan 18, feb 17, mar 17,apr 15
static int loc=15;

int deltaMoon1984 (int conDate){
	if(conDate < 16){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1984(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1984.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1984.csv",ios::app);
term.open("termPreg1984.csv",ios::app);
gest.open("oldTerm1984.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1983
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1984(conDate);
