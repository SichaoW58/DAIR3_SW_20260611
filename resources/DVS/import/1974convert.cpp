//1974convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {18/*n*/,48,77,107,137,166/*n*/,196/*n*/,226,255,285,314,344/*p*/,373,402,432,461};//jan 1, 1973 = 1, these are the dates of full moons
//jan 18, feb 17,mar 18,apr 17,may 17,jun 15,jul 15,aug 14,sep 12,oct 12,nov 10,dec 10, jan 8, feb 6,mar 8, apr 6
static int loc=15;

int deltaMoon1974 (int conDate){
	if(conDate < 19){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1974(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1974.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1974.csv",ios::app);
term.open("termPreg1974.csv",ios::app);
gest.open("oldTerm1974.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1973
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1974(conDate);
