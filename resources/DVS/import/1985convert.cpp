//1985convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {/*18,*/48,77,106,136/*n*/,165/*n*/,195,224,254,283,313/*n*/,343,373,402,432,461,490/*t*/};//jan 1, 1984 = 1, these are the dates of full moons
//Jan 18, feb 17, mar 17,apr 15, may 15,jun 13, jul 13,aug 11,sep 10,oct 9,nov 8,dec 8,jan 7, feb 5,mar 7,apr 5,may 4
static int loc=15;

int deltaMoon1985 (int conDate){
	if(conDate < 49){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1985(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1985.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1985.csv",ios::app);
term.open("termPreg1985.csv",ios::app);
gest.open("oldTerm1985.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 366;//we shift the dates on the dates of birth so that 1= Jan 1, 1984 and 1984 is a leap year
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1985(conDate);
