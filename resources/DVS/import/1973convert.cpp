//1973convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {30/*t*/,60,89,119,149,178,208/*p*/,237,267,296,325,355,384/*n*/,414,443,473};//jan 1, 1972 = 1, these are the dates of full moons
//jan 30, feb 29, mar 29,apr 28,may 28,jun 26,jul 26,aug 24,sep 23,oct 22,nov 20,dec 20,jan 18,feb 17,mar 18,apr 17
static int loc=15;

int deltaMoon1973 (int conDate){
	if(conDate < 31){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1973(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1973.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1973.csv",ios::app);
term.open("termPreg1973.csv",ios::app);
gest.open("oldTerm1973.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 366;//we shift the dates on the dates of birth so that 1= Jan 1, 1972
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1973(conDate);
