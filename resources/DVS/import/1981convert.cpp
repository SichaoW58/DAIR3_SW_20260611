//1981convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {/*2,*/32,61/*n*/,91,121,150,180,209/*n*/,239/*n*/,268,297,327,356,386/*n*/,415,445,475};//jan 1, 1980 = 1, these are the dates of full moons
//jan 2,feb 1,mar 1,mar 31,apr 30,may 29,jun 28,jul 27,aug 26,sep 24,oct 23,nov 22,dec 21,jan 20, feb 18, mar 20, apr 19
static int loc=15;

int deltaMoon1981 (int conDate){
	if(conDate < 49){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1981(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1981.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1981.csv",ios::app);
term.open("termPreg1981.csv",ios::app);
gest.open("oldTerm1981.csv",ios::app);
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		switch (birthMonth){
			...
			case 12:
				...
				break;
			default:
				birthDate = -367;//birthMonth is 99
				break;
		}
		//cout << birthDate << endl;
		birthDate += 366;//we shift the dates on the dates of birth so that 1= Jan 1, 1980 and 1980 is a leap year
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1981(conDate);
