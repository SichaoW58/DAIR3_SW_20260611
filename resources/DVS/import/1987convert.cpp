//1987convert.cpp by: Derek Onken
//
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {26,55,85,114/*t*/,143,173,202,231,261,290/*t*/,320,350,380,409,439,469/*n*/};//jan 1, 1986 = 1, these are the dates of full moons
//jan 26,feb 24,mar 26, apr 24,may 23,jun 22,jul 21,aug 19,sept 18,oct 17, nov 16,dec 16,jan 15,feb 13,mar 15,apr 14
static int loc=15;

int deltaMoon1987 (int conDate){
	if(conDate < 27){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1987(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1987.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1987.csv",ios::app);
term.open("termPreg1987.csv",ios::app);
gest.open("oldTerm1987.csv",ios::app);
...
while(file.good() ){
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1986
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1987(conDate);
