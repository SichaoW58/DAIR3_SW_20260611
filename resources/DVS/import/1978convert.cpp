//1978convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {/*5,*/35,64,94/*p*/,123,152,182,211,240,270/*n*/,299,329,359,389,419,448/*t*/,478};//jan 1, 1977 = 1, these are the dates of full moons
//jan 5,feb 4,mar 5,apr 4,may 3,jun 1,jul 1,jul 30,aug 28,sep 27,oct 26,nov 25,dec 25,jan 24,feb 23,mar 24,apr 23
static int loc=15;

int deltaMoon1978 (int conDate){
	if(conDate < 36){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1978(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1978.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1978.csv",ios::app);
term.open("termPreg1978.csv",ios::app);
gest.open("oldTerm1978.csv",ios::app);
...
csv << "Year of birth,shipment number,reporting area,blank(certificate #),resident(1=yes 2=no),resident status,place of residence," <<
		...
		"mother's birthplace,total number of prenatal visits,congenital malformation,Indian Community Occurence Code,flags regarding place of residence," <<
		...
		"number prenatal visits recode12, number of days that conception date occurs after most recent full moon,\n";
...
while(file.good()  ){
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1977
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1978(conDate);
