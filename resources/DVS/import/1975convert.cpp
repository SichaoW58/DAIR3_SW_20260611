//1975convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {8,37,67,96,126,155/*p*/,185,215,244,274,304,333/*t*/,363,392,422,451};//jan 1, 1974 = 1, these are the dates of full moons
//jan 8,feb 6,mar 8,apr 6,may 6,jun 4, jul 4, aug 3,sep 1,oct 1,oct 31,nov 29,dec 29,jan 27,feb 26,mar 27
static int loc=15;

int deltaMoon1975 (int conDate){
	if(conDate < 9){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1975(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1975.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1975.csv",ios::app);
term.open("termPreg1975.csv",ios::app);
gest.open("oldTerm1975.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1974
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1975(conDate);
