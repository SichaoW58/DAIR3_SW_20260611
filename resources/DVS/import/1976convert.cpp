//1976convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {27,57,86,115,145/*t*/,174,204,233,263,293,322/*t*/,352,382,411,441,470};//jan 1, 1975 = 1, these are the dates of full moons
//jan 27,feb 26,mar 27,apr 25,may 25,jun 23,jul 23,aug 21,sep 20,oct 20,nov 18,dec 18,jan 17,feb 15,mar 16,apr 14
static int loc=15;

int deltaMoon1976 (int conDate){
	if(conDate < 28){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1976(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1976.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1976.csv",ios::app);
term.open("termPreg1976.csv",ios::app);
gest.open("oldTerm1976.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1975
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1976(conDate);
