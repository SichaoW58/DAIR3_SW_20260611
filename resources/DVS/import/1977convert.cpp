//1977convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {17,46,76,105,134/*p*/,164,193,222,252,282,311,341,371,401,430,460/*p*/};//jan 1, 1976 = 1, these are the dates of full moons
//jan 17,feb 15,mar 16,apr 14,may 13,jun 12,jul 11,aug 9,sep 8,oct 8,nov 6,dec 6,jan 5,feb 4,mar 5,apr 4
static int loc=15;

int deltaMoon1977 (int conDate){
	if(conDate < 18){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1977(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1977.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1977.csv",ios::app);
term.open("termPreg1977.csv",ios::app);
gest.open("oldTerm1977.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 366;//we shift the dates on the dates of birth so that 1= Jan 1, 1976
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1977(conDate);
