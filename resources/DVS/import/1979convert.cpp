//1979convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {24,54,83/*t*/,113,142,171,201,230,259/*t*/,289,318,348,378,408,437/*p*/,467};//jan 1, 1978 = 1, these are the dates of full moons
//jan 24,feb 23,mar 24,apr 23,may 22,jun 20,jul 20,aug 18,sep 16,oct 16,nov 14,dec 14,jan 13,feb 12,mar 13,apr 12
static int loc=15;

int deltaMoon1979 (int conDate){
	if(conDate < 25){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1979(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1979.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1979.csv",ios::app);
term.open("termPreg1979.csv",ios::app);
gest.open("oldTerm1979.csv",ios::app);
...
//	x++;
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1978
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1979(conDate);
