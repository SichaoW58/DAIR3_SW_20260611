//1980convert.cpp by: Derek Onken
//
#include <fstream>
#include <string>
#include <iostream>
...
static int error3 =0;
static int error4 =0;
static int fulls[16] = {13,43,72/*p*/,102,132,161,190,220,249/*t*/,278,308,337,367,397,426/*n*/,456};//jan 1, 1979 = 1, these are the dates of full moons
//jan 13,feb 12, mar 13,apr 12,may 12, jun 10,jul 9,aug 8,sep 6,oct 5,nov 4,dec 3,jan 2,feb 1,mar 1,mar 30
static int loc=15;

int deltaMoon1980 (int conDate){
	if(conDate < 14){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1980(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1980.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1980.csv",ios::app);
term.open("termPreg1980.csv",ios::app);
gest.open("oldTerm1980.csv",ios::app);
...
csv << "Year of birth,shipment number,reporting area,blank(certificate #),resident(1=yes 2=no),resident status,place of residence," <<
		...
		"one minute apgar,one min. apgar recode5,five minute apgar, five min. apgar recode5,origin descent mother, origin descent father,reserved (used after 1981)," <<
		"Fed Info Processing Standards SMSA,record weight(2=50% data submitted),number prenatal vists recode28," <<
...
while(file.good() ){
	...
	*/
	csv <<  "198" <<  raw[0] << ","//year of birth
		...
		<< raw[87] << raw[88] << raw[89] << raw[90];
		if ((raw[91]-'0') == 0){
			csv << "198"  << raw[91] << ","; //date of last menstrual period MMDDYYYY
		}
		else if ((raw[91]-'0') == 9){
			csv << "197"  << raw[91] << ","; //date of last menstrual period MMDDYYYY
		}
		else{
			csv << "19-" << raw[91] << ",";
		}
		csv << raw[92] << raw[93] << "," //gestation in weeks
		...
		<< raw[212] << raw[213] << raw[214] << ","; //reserved
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1980 and 1980 is a leap year
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1980(conDate);
		...
		<< raw[87] << raw[88] << raw[89] << raw[90];
		if ((raw[91]-'0') == 0){
			term << "198"  << raw[91] << ","; //date of last menstrual period MMDDYYYY
		}
		else if ((raw[91]-'0') == 9){
			term << "197"  << raw[91] << ","; //date of last menstrual period MMDDYYYY
		}
		else{
			term << "19-" << raw[91] << ",";
		}
		term << raw[92] << raw[93] << "," //gestation in weeks
		<< raw[94] << raw[95] << "," //gestation recode 10
		<< raw[96] << ","//gestation recode 3
		<< raw[97] << raw[98] << ","//mother education
		<< raw[99] << raw[100] << ","//mother education recode 14
