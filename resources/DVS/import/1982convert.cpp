//1982convert.cpp by: Derek Onken
//
...
static int fulls[16] = {20/*n*/,49,79,109,139,168,198/*p*/,227,257,286,315,345,374/*t*/,404,433,463};//jan 1, 1981 = 1, these are the dates of full moons
//Jan 20, feb 18, mar 20,apr 19,may 19,jun  17, jul 17,aug 15,sep 14,oct 13,nov 11, dec 11,jan 9, feb 8, mar 9,apr 8
static int loc=15;

int deltaMoon1982 (int conDate){
	if(conDate < 37){
		error1++;
		if (conDate<0){
			error3++;
	...
	int deltaMoon = fulls[loc] - conDate;
			if (deltaMoon > 0){
				loc--;
				deltaMoon = deltaMoon1982(conDate);
			}
			loc=15;
	return deltaMoon;
...
file.open("NATL1982.txt");
//file.open("TestTxt.txt");
csv.open("TestResults1982.csv",ios::app);
term.open("termPreg1982.csv",ios::app);
gest.open("oldTerm1982.csv",ios::app);
...
while(file.good()  ){
	...
	*/
	...
	if (gestWeek >=37 && gestWeek<=41){
		...
		//cout << birthDate << endl;
		birthDate += 365;//we shift the dates on the dates of birth so that 1= Jan 1, 1981
		int conDate;
		gest << birthDate << ",";
		...
		int deltaMoon = deltaMoon1982(conDate);
